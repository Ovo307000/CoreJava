...
public class GridBagPane extends JPanel
{
	...
	public Component get(String name)
	{
		...
		for (int i = 0; i < components.length; i++)
		{
			if (components[i].getName()
			                 .equals(name)) return components[i];
		}
		return null;
	}
	...
	private Object parseBean(Element elem)
	{
		try
		{
			...
			Object obj = cl.getDeclaredConstructor()
			               .newInstance();
			...
			for (int i = 1; i < children.getLength(); i++)
			{
				...
				{
					if (descriptors[j].getName()
					                  .equals(propertyName))
					{
						descriptors[j].getWriteMethod()
						              .invoke(obj, value);
						done = true;
					}
				}
	...
	private Object parseValue(Element elem)
	{
		...
		if (child.getTagName()
		         .equals("bean")) return parseBean(child);
		String text = ((Text) child.getFirstChild()).getData();
		if (child.getTagName()
		         .equals("int")) return Integer.valueOf(text);
		else if (child.getTagName()
		              .equals("boolean")) return Boolean.valueOf(text);
		else if (child.getTagName()
		              .equals("string")) return text;
		else return null;
	}
}
